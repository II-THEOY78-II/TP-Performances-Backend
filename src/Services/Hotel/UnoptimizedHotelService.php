...
class UnoptimizedHotelService extends AbstractHotelService {
  ...
  protected function getMetas ( HotelEntity $hotel ) : array {
    $timer = Timers::getInstance();
    $timerId = $timer->startTimer('TestgetMetas');
    $metaDatas = [
      'address' => [
    ...
    $timer->endTimer('TestgetMetas', $timerId);
    return $metaDatas;
  }
  ...
  protected function getCheapestRoom ( HotelEntity $hotel, array $args = [] ) : RoomEntity {
    ...
    foreach ( $filteredRooms as $room ) :
      ...
      if ( intval( $room->getPrice() ) < intval( $cheapestRoom->getPrice() ) )
        $cheapestRoom = $room;
    endforeach;
    $timer->endTimer('TestgetCheapestRoom', $timerId);
    return $cheapestRoom;
  }
  ...
  protected function convertEntityFromArray ( array $data, array $args ) : HotelEntity {
    $timer = Timers::getInstance();
    $timerId = $timer->startTimer('TestConvertEntityFromArray');
    ...
    $timer->endTimer('TestConvertEntityFromArray', $timerId);
    return $hotel;
  }
  ...
  public function list ( array $args = [] ) : array {
    $timer = Timers::getInstance();
    $timerId = $timer->startTimer('TestList');
    ...
    $timer->endTimer('TestList', $timerId);
    return $results;
  }
